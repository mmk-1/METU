...
bool Flight::addTicket(const Ticket &ticket)
{
    ...
    int economySize = 0, businessSize = 0;
    ...
    if (economySize == this->economyCapacity && ticket.getTicketType() == economy)
        return false;
    if(businessSize == this->businessCapacity && ticket.getTicketType() == business)
        return false;
    // if (ticket.getTicketType() == economy)
    // {
    //     economySize++;
    //     if (economySize == this->economyCapacity && businessSize == this->businessCapacity)
    //         setCompleted(true);
    // }
...
{
    /* TODO */
    std::string lhsFlightCode = flightCode, rhsFlightCode = rhs.flightCode;
    
    for (int i = 0; i < (int) lhsFlightCode.size(); i++)
    {
        lhsFlightCode[i] = tolower(lhsFlightCode[i]);
    }
    for (int i = 0; i < (int) rhsFlightCode.size(); i++)
    {
        rhsFlightCode[i] = tolower(rhsFlightCode[i]);
    }
    if (lhsFlightCode < rhsFlightCode)
    {
        return true;
    }
    else
