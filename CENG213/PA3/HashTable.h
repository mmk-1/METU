...
// for deleted entries
static const std::string REMOVED =  "__DELETED__" ;
...
struct HashData
{
    ...
    // Constructor
                        HashData() { key = ""; }
    private:
        bool operator!=(const HashData &rhs) {
            return (this->intArray != rhs.intArray && this->key != rhs.key);
        }
};
...
class KeyedHashTable
{
    private:
        ...
        //custom funcs
        int pow(int base, int exp) const;
        bool Contains(const std::string &key) const;
        bool operator!=(const KeyedHashTable &rhs){
